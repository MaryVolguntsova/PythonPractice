    ...
    while True:
        ...
        match choice:
            ...
            case 7:
                ...
                #     if len(result) != 1:
                #         view.print_contacts(result, '')
                #         current_id = view.input_index(text.input_index)
                #     else:
                #         current_id = result[0].get('id')
                #     name = model.delete_contact(current_id)
                #     if name:
                #         view.print_message(text.delete_successful.format(name))
                #     else:
                #         view.print_message(text.empty_search(current_id))
                # else:
                #     view.print_message(text.empty_search(key_word))
                key_word = view.input_search(text.input_delete)
                result = model.search_contact(key_word)
                if result:
                    if len(result) != 1:
                        view.print_contacts(result, '')
                        current_id = str(view.input_index(text.input_index))
                    else:
                        current_id = result[0].get('id')
                    name = model.delete_contact(current_id)
                    if name:
                    ...
                    view.print_message(text.empty_search(key_word))
            case 8:
                save_before_exit = view.ask_save_before_exit()
                if save_before_exit:
                    model.save_pb()
                break